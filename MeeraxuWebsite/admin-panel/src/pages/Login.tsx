        ...
        <div className="bg-white/95 backdrop-blur-[20px] rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.3),inset_0_0_1px_rgba(255,255,255,0.5)] p-8 sm:px-10 py-8 border border-white/80 animate-[slideIn_0.6s_ease-out] w-full">
          <div className="mb-[18px]">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-11 h-11 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl flex items-center justify-center text-2xl font-extrabold text-white shadow-[0_8px_24px_rgba(59,130,246,0.3)]">M</div>
              <div>
                <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight m-0">MEERAXU</h1>
                <p className="text-xs text-slate-500 font-semibold uppercase tracking-[1px] mt-0.5">Admin Portal</p>
              </div>
            </div>
          </div>

          <div className="mb-[18px]">
            <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-1.5">Welcome Back</h2>
            <p className="text-[13px] text-slate-500 mb-3.5 font-medium">
              Sign in to your admin account
            </p>

            {error && <div className="bg-red-100 text-red-800 px-4 py-3 rounded-xl mb-5 text-sm font-semibold border-l-4 border-red-600 animate-[slideIn_0.3s_ease-out]">{error}</div>}

            <form onSubmit={handleSubmit} className="mb-6">
              <div className="mb-[18px]">
                <label htmlFor="email" className="block text-xs font-bold text-slate-800 mb-2 uppercase tracking-[0.8px]">Email Address</label>
                <div className="relative flex items-center">
                  <span className="absolute left-3.5 text-lg pointer-events-none text-slate-500">✉️</span>
                  <input
                    id="email"
                    type="email"
              ...
              <div className="mb-[18px]">
                <label htmlFor="password" className="block text-xs font-bold text-slate-800 mb-2 uppercase tracking-[0.8px]">Password</label>
                <div className="relative flex items-center">
                  <span className="absolute left-3.5 text-lg pointer-events-none text-slate-500">🔐</span>
                  <input
                    id="password"
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 text-[13px] gap-3">
                <label className="flex items-center cursor-pointer text-slate-600 font-medium select-none">
                  <input type="checkbox" className="w-[18px] h-[18px] mr-2 cursor-pointer accent-blue-500 rounded" />
                  <span>Remember me for 30 days</span>
                </label>
              </div>

              <button type="submit" className="w-full py-3 px-6 bg-gradient-to-br from-blue-500 to-blue-600 text-white border-none rounded-xl text-sm font-bold cursor-pointer transition-all duration-300 flex items-center justify-center gap-2 tracking-[0.5px] shadow-[0_8px_20px_rgba(59,130,246,0.3)] uppercase mb-4 hover:not-disabled:-translate-y-[2px] hover:not-disabled:shadow-[0_12px_28px_rgba(59,130,246,0.4)] hover:not-disabled:from-blue-600 hover:not-disabled:to-blue-700 active:not-disabled:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none group" disabled={loading}>
                {loading ? (
                  <>
          ...
          <div className="text-center pt-3 border-t border-slate-200">
            <p className="text-slate-400 text-[11px] m-0 font-medium">© 2026 MEERAXU. All rights reserved.</p>
          </div>
        </div>
      </div>
      <style>{`
