...
const Dashboard = () => {
  ...
  const [counts, setCounts] = useState({ services: 0, projects: 0, leads: 0 });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-slate-50 min-h-screen font-sans max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-8 gap-4 sm:gap-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 m-0 mb-1 tracking-tight">
            Dashboard
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 mb-8">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white rounded-xl p-5 sm:p-6 shadow-sm border border-slate-200 flex flex-col sm:flex-row lg:flex-col items-start gap-4 sm:gap-0 relative overflow-hidden cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:shadow-md hover:border-blue-500 group"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-col lg:flex-row items-start lg:justify-between mb-0 sm:mb-0 lg:mb-5 w-full">
              <div
                className={`w-12 h-12 sm:w-14 sm:h-14 lg:w-20 lg:h-20 rounded-xl border-2 flex items-center justify-center text-2xl lg:text-4xl shrink-0 transition-transform duration-300 group-hover:scale-105 ${stat.color}`}
              >
                {stat.icon}
              </div>
            </div>
            ...
            <div className="flex-1 w-full">
              <p className="text-[11px] lg:text-xs text-slate-500 mb-1 lg:mb-2 uppercase tracking-wide font-bold">
                {stat.label}
              </p>
              <h3 className="text-2xl lg:text-4xl font-extrabold text-slate-900 m-0 tracking-tight">
                {stat.value}
              </h3>
            </div>
          </div>
